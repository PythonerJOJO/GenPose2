...
def get_ckpt_and_writer_path(cfg):
    ...
    writer_path = (
        f"./results/logs/{cfg.log_dir}"
        if cfg.use_pretrain == False
        else f"./results/logs/{cfg.log_dir}_continue"
    )
...
class PoseNet(nn.Module):
    def __init__(self, cfg):
        ...
        # init sde
        self.prior_fn, self.marginal_prob_fn, self.sde_fn, self.sampling_eps, self.T = (
            init_sde(self.cfg.sde_mode)
        )
        self.net = self.build_net()
        self.optimizer = self.set_optimizer()
        self.scheduler = self.set_scheduler()
        self.ema = ExponentialMovingAverage(
            self.net.parameters(), decay=self.cfg.ema_rate
        )

        # init related functions
        if self.cfg.sde_mode == "edm":
            self.loss_fn = functools.partial(
                loss_fn_edm, sigma_data=1.4148, P_mean=-1.2, P_std=1.2
            )
        else:
        ...
        if name == "GFObjectPose":
            ...
            return ScaleNet(
                ...
                dino_dim=(
                    ...
                    else GFObjectPose.dino_dim + GFObjectPose.embedding_dim
                ),
    ...
    def set_optimizer(self):
        ...
        if self.cfg.optimizer == "SGD":
            optimizer = optim.SGD(
                params, lr=self.cfg.lr, momentum=0.9, weight_decay=1e-4
            )
    ...
    def load_ckpt(
        self, name=None, model_dir=None, model_path=False, load_model_only=False
    ):
    ...
    def collect_ranking_loss(self, data, pred_pose):
        energy = self.get_energy(
            data=data, pose_samples=pred_pose, mode="train", extract_pts_feature=False
        )
        ...
        for key in ["gt_pose", "id", "sym_info"]:
            ...
            repeated_data[key] = (
                data[key].unsqueeze(1).repeat(repeat_list).view(bs * repeat_num, -1)
            )
    ...
    def collect_scale_loss(self, data):
        ...
        _data = {
            ...
            "pts_feat": data["pts_feat"].repeat_interleave(
                self.cfg.scale_batch_size, dim=0
            ),
        }
        if "rgb_feat" in data and data["rgb_feat"] is not None:
            _data["rgb_feat"] = data["rgb_feat"].repeat_interleave(
                self.cfg.scale_batch_size, dim=0
            )
    ...
    def train_score_func(self, data, teacher_model=None):
        ...
        with torch.no_grad():  # 没有教师模型
            ...
            pts_feat_teacher = (
                ...
                else teacher_model(data, mode="pts_feature")
            )
        self.pts_feature = True
        # 在这里面解码
    ...
    def eval_score_func(self, data, data_mode):
        ...
        with torch.no_grad():
            ...
            for res_item, sampler_item in zip(res_list, sampler_mode_list):
                metric = self.collect_pose_metric(
                    res_item, data["gt_pose"], data["sym_info"]
                )
            ...
            if self.cfg.save_video:
                ...
                test_time_visulize(
                    save_path, data, res, in_process_sample, self.cfg.pose_mode
                )
            self.pts_feature = False
        self.ema.restore(self.net.parameters())
    ...
    def eval_scale_func(self, data, data_mode):
        ...
        with torch.no_grad():
            ...
            _data = data.copy()
            _data["axes"] = _data["axes_training"][
                :, 0, :, :
            ]  # no need for additional batching in evaluation
            pred_len = self.net(_data)

            metric = self.collect_length_metric(
                pred_len, data["length_training"][:, 0, :]
            )
    ...
    def pred_func(
        ...
        with torch.no_grad():
            ...
            in_process_sample, res = self.net(
                repeated_data,
                mode=f"{self.cfg.sampler_mode[0]}_sample",
                init_x=repeated_init_x,
                T0=T0,
            )
            pred_pose = res.reshape(bs, repeat_num, -1)
            in_process_sample = in_process_sample.reshape(
                bs, repeat_num, in_process_sample.shape[1], -1
            )
            ...
            pred_pose_q_wxyz = res_q_wxyz.reshape(
                bs, repeat_num, -1
            )  # [bs, repeat_num, 7]
            """ Calculate the average results """
            if return_average_res:
                average_pred_pose_q_wxyz = torch.zeros((bs, 7)).to(
                    pred_pose_q_wxyz.device
                )
                average_pred_pose_q_wxyz[:, :4] = average_quaternion_batch(
                    pred_pose_q_wxyz[:, :, :4]
                )
                average_pred_pose_q_wxyz[:, 4:] = torch.mean(
                    pred_pose_q_wxyz[:, :, 4:], dim=1
                )
                if return_process:
                    return (
                        pred_pose,
                        pred_pose_q_wxyz,
                        average_pred_pose_q_wxyz,
                        in_process_sample,
                    )
                else:
    ...
    def get_energy(self, data, pose_samples, T=None, mode="test", extract_feature=True):
        ...
        if mode == "train":
            pts_feat = (
                ...
                else self.net(data, mode="pts_feature")
            )
            rgb_feat = (
                data["rgb_feat"]
                if extract_feature == False
        ...
        repeat_list[1] = repeat_num
        repeated_pts_feat = (
            pts_feat.unsqueeze(1).repeat(repeat_list).view(bs * repeat_num, -1)
        )
        ...
        if self.cfg.dino == "global":
            ...
            repeat_list[1] = repeat_num
            repeated_rgb_feat = (
                rgb_feat.unsqueeze(1).repeat(repeat_list).view(bs * repeat_num, -1)
            )
        else:
        ...
        energy_input_data = {
            ...
            "sampled_pose": pose_samples.clone()
            .view(bs * repeat_num, -1)
            .type_as(repeated_pts_feat),
        }
        if T is not None:
            energy_input_data["t"] = (
                torch.ones(bs * repeat_num, 1).type_as(repeated_pts_feat) * T
            )
        else:
            T_min = 1e-5
            T_max = 1e-4
            T_samples = (
                torch.randint(int(T_min * 1e5), int(T_max * 1e5), (bs, 1)).type_as(
                    repeated_pts_feat
                )
                / 1e5
            )
    ...
    def update_network(self, loss_dict):
        ...
        if self.cfg.grad_clip >= 0:
            torch.nn.utils.clip_grad_norm_(
                self.net.parameters(), max_norm=self.cfg.grad_clip
            )
        self.optimizer.step()
    ...
    def update_learning_rate(self):
        ...
        if self.clock.step <= self.cfg.warmup:
            self.optimizer.param_groups[-1]["lr"] = (
                self.base_lr / self.cfg.warmup * self.clock.step
            )
    ...
    def record_metrics(self, metric, sampler_mode, mode="val"):
            ...
            for k, v in rot_error.items():
                if not k == "item":
                    self.writer.add_scalar(
                        f"{mode}/{sampler_mode}_{k}_rot_error", v, self.clock.epoch
                    )
        ...
        if "trans_error" in metric:
            ...
            for k, v in trans_error.items():
                if not k == "item":
                    self.writer.add_scalar(
                        f"{mode}/{sampler_mode}_{k}_trans_error", v, self.clock.epoch
                    )
        ...
        if "length_error" in metric:
            ...
            for k, v in length_error.items():
                if not k == "item":
                    self.writer.add_scalar(
                        f"{mode}/{sampler_mode}_{k}_length_error", v, self.clock.epoch
                    )

    def record_lr(self):
        self.writer.add_scalar(
            "learing_rate", self.optimizer.param_groups[0]["lr"], self.clock.step
        )
        ...
        for res_item, sampler_item in zip(res, sampler_mode):
            ...
            if "color" in data.keys():
                grid_image, _ = create_grid_image(
                    pts, res_item, data["gt_pose"], data["color"], self.cfg.pose_mode
                )
            else:
                grid_image, _ = create_grid_image(
                    pts, res_item, data["gt_pose"], None, self.cfg.pose_mode
                )
            self.writer.add_image(
                f"{mode}/vis_{sampler_item}", grid_image, self.clock.epoch
            )
    ...
    def collect_pose_metric(self, pred_pose, gt_pose, sym_info):
        ...
        return {"rot_error": rot_error, "trans_error": trans_error}
