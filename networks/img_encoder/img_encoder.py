...
class ImgEncoder(nn.Module):
    def __init__(self, dim, num_patches, patch_size):
        super().__init__()
        self.dim = dim
        ...
        self.geo_weight = nn.Parameter(torch.tensor(0.2))  # 空间几何特征权重
        self.edge_weight = nn.Parameter(torch.tensor(0.1))  # 边缘增强特征权重
    ...
    def forward(self, features):
        ...
        final_feats = (
            fused_feats
            + F.relu(self.geo_weight) * geo_enhanced_feats
            + F.relu(self.edge_weight) * edge_enhanced_feats
        )
