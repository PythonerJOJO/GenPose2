...
class PoseScoreNet(nn.Module):
    def __init__(
        ...
        elif self.regression_head == "Rx_Ry_and_T":  # True
            if pose_mode != "rot_matrix":
                raise NotImplementedError
            if per_point_feature:
                self.fusion_tail_rot_x = RotHead(
                    in_feat_dim=128 + 256 + 1280, out_dim=3
                )  # t_feat + pose_feat + pts_feat
                self.fusion_tail_rot_y = RotHead(
                    in_feat_dim=128 + 256 + 1280, out_dim=3
                )  # t_feat + pose_feat + pts_feat
                self.fusion_tail_trans = TransHead(
                    in_feat_dim=128 + 256 + 1280, out_dim=3
                )  # t_feat + pose_feat + pts_feat
            else:
    ...
    def forward(self, data):
        ...
        sampled_pose = data["sampled_pose"]  # R ([32, 9])
        t = data["t"]  # t ([32, 1])
        ...
        if self.regression_head == "RT":
            ...
            trans = self.fusion_tail_trans(total_feat)
            out_score = torch.cat([rot_x, rot_y, trans], dim=-1) / (
                std + 1e-7
            )  # normalisation
            ...
            trans = self.fusion_tail_trans(total_feat)
            out_score = torch.cat([angle, vec, trans], dim=-1) / (
                std + 1e-7
            )  # normalisation
        else:
            raise NotImplementedError
