...
def loss_fn_edm(
    ...
    sigma_t = (
        math.log(sigma_min) + log_sigma_t * (math.log(sigma_max) - math.log(sigma_min))
    ).exp()  # [bs, 1]
...
def loss_fn(
    ...
    """ get std """
    bs = pts.shape[0]
    random_t = torch.rand(bs, device=device) * (1.0 - eps) + eps  # [bs, ]
    ...
    loss_ = torch.mean(
        torch.sum(
            (loss_weighting * (estimated_score - target_score) ** 2).view(bs, -1),
            dim=-1,
        )
    )
