...
def ve_sde(t, sigma_min=0.01, sigma_max=90):
    ...
    diffusion_coeff = sigma * torch.sqrt(
...
def ve_prior(shape, sigma_min=0.01, sigma_max=90, T=1.0):
    _, sigma_max_prior = ve_marginal_prob(
        None, T, sigma_min=sigma_min, sigma_max=sigma_max
    )
...
def init_sde(sde_mode):
    ...
    if sde_mode == "edm":
        ...
        prior_fn = functools.partial(
            edm_prior, sigma_min=sigma_min, sigma_max=sigma_max
        )
        marginal_prob_fn = functools.partial(
            edm_marginal_prob, sigma_min=sigma_min, sigma_max=sigma_max
        )
        ...
        eps = 1e-5
        marginal_prob_fn = functools.partial(
            ve_marginal_prob, sigma_min=sigma_min, sigma_max=sigma_max
        )
        ...
        marginal_prob_fn = functools.partial(
            vp_marginal_prob, beta_0=beta_0, beta_1=beta_1
        )
        ...
        marginal_prob_fn = functools.partial(
            subvp_marginal_prob, beta_0=beta_0, beta_1=beta_1
        )
