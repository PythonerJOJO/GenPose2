...
def cond_ode_likelihood(
    ...
    def divergence_eval(data, epsilon):
        ...
        with torch.enable_grad():
            ...
            grad_score_energy = torch.autograd.grad(score_energy, data["sampled_pose"])[
                0
            ]  # [bs, pose_dim]
        # reset sampled_pose
    ...
    res = integrate.solve_ivp(
        ode_func, (eps, 1.0), init_inp, rtol=rtol, atol=atol, method="RK45"
    )
...
def cond_pc_sampler(
    ...
    with torch.no_grad():
        for time_step in time_steps:
            batch_time_step = (
                torch.ones(batch_size, device=device).unsqueeze(-1) * time_step
            )
