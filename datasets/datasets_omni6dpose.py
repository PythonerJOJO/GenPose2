...
class Omni6DPoseDataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        if not self.cfg.load_per_object:  # True
            img_path = self.img_list[index // self.REPCNT]
            gts: ImageMetaData = Dataset.load_meta(img_path + "meta.json")
            if not any(obj.is_valid for obj in gts.objects):  # False
            ...
            if self.per_obj:
                ...
                if select_idx < self.REPCNT - self.REPCNT % len(valid_objects):  # True
        ...
        if not (mask.shape[:2] == depth.shape[:2] == rgb.shape[:2]):  # False
            assert 0, "invalid data"

        intrinsics = gts.camera.intrinsics  # 相机内参
        ...
        if inst_name not in self.obj_meta.instance_dict:  # False
            return self.__getitem__((index + 1) % self.__len__())
        ...
        if not np.any(object_mask):  # False
            return self.__getitem__((index + 1) % self.__len__())
        ys, xs = np.argwhere(object_mask).transpose(1, 0)  # 目标对象掩码的所有y,x坐标
        rmin, rmax, cmin, cmax = (
            np.min(ys),
            np.max(ys),
            np.min(xs),
            np.max(xs),
        )  # 对象 4 个角，row对应图像坐标里的 y 轴；column对应图像坐标里的 x 轴
...
def process_batch(batch_sample, device, pose_mode="quat_wxyz", PTS_AUG_PARAMS=None):
    ...
    if PTS_AUG_PARAMS == None:
        ...
        PC_da = batch_sample["pcl_in"].to(device)  # 1024个采样的点云
        gt_R_da = batch_sample["rotation"].to(device)
        gt_t_da = batch_sample["translation"].to(device)
    ...
    if "axes_training" in batch_sample:  # False
