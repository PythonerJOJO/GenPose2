...
from datasets.datasets_omni6dpose import (
    Omni6DPoseDataSet,
    array_to_SymLabel,
...
def inference_energy(score_path, save_path):
    ...
    energy_agent.load_ckpt(
        model_dir=cfg.pretrained_energy_model_path,
        model_path=True,
...
def aggregate_pose(score_path, energy_path, save_path):
    ...
    for i, (pred_pose, pred_energy) in enumerate(
        tqdm(zip(all_pred_pose, all_pred_energy), desc="aggregate")
    ):
...
def inference_scale(score_path, aggregate_path, save_path):
    ...
    for i, test_batch in enumerate(tqdm(dataloader, desc="scale")):
        ...
        batch_sample.update(
            {
                key: (None if value is None else value.to(cfg.device))
...
def get_detect_match(cls_path, save_path):
    ...
    for i, test_batch in enumerate(tqdm(dataloader, desc="detect match")):
        ...
        dm = DetectMatch(
            ...
            image_path=[path for path in test_batch["path"]],
...
def print_metrics(dm_path, criterion_path, save_path):
    ...
    criterion: "tuple[np.ndarray, np.ndarray, np.ndarray]" = pickle.load(
        open(criterion_path, "rb")
    )
...
def visualize_pose_distribution(score_path, dm_path):
    ...
    for i, test_batch in enumerate(tqdm(dataloader)):
        ...
        pose_rot = get_rot_matrix(
            pred_pose.reshape(pred_pose.shape[0] * cfg.eval_repeat_num, -1),
            cfg.pose_mode,
