...
from datasets.datasets_tracking import (
    Omni6DPoseDataSet,
    array_to_SymLabel,
    array_to_CameraIntrinsicsBase,
    process_batch,
)
...
def get_dataloader(data_dir: str):
    ...
    data_loaders = DatasetPort.get_data_loaders_from_cfg(
        cfg, dataset_type="xyzibd", data_type=["val"]
    )
    dataloader = data_loaders["val_loader"]
    print("val_set: ", len(dataloader))
...
score_agent.load_ckpt(
    model_dir=cfg.pretrained_score_model_path, model_path=True, load_model_only=True
)
score_agent.eval()
...
energy_agent.load_ckpt(
    model_dir=cfg.pretrained_energy_model_path, model_path=True, load_model_only=True
)
energy_agent.eval()
...
if cfg.pretrained_scale_model_path:
    ...
    scale_agent.load_ckpt(
        model_dir=cfg.pretrained_scale_model_path, model_path=True, load_model_only=True
    )
    scale_agent.eval()
...
def work_batch(test_batch, prev_pose):
    ...
    score_feature = {
        ...
        "rgb_feat": (
            ...
            else batch_sample["rgb_feat"].clone()
        ),
    }
    ...
    rot_matrix = get_rot_matrix(
        good_pose[:, :, :-3].reshape(bs * retain_num, -1), cfg.pose_mode
    )
    ...
    if cfg.clustering:
        for j in range(bs):
            ...
            dbscan = DBSCAN(
                ...
                min_samples=int(cfg.clustering_minpts * retain_num),
    ...
    detect_match = DetectMatch(
        ...
        pred_affine=pred_pose,
        pred_size=pred_length,
    ...
    prev_pose[:, :-3] = get_pose_representation(
        aggregated_pose[:, :3, :3], cfg.pose_mode
    )
...
while 1:
    ...
    for i, dataloader in enumerate(dataloaders):
        try:
            ...
            if len(
                dataloader.all_detect_match
            ):  # otherwise, an error occurred in the dataset
        ...
        try:
            ...
            pose = torch.zeros(
                length, get_pose_dim(cfg.pose_mode), device=cfg.device
            )  # on gpu
            assert batch["affine"].shape[0] == length, set_trace()
            for j in range(length):
                noise_gt_pose = add_noise_to_RT(
                    batch["affine"][j].to(cfg.device).unsqueeze(0)
                )[0]
                pose[j, :-3] = get_pose_representation(
                    noise_gt_pose[:3, :3].unsqueeze(0), pose_mode=cfg.pose_mode
                )[0]
...
all_crit = [
    np.concatenate([all_crit[j][i] for j in range(len(all_crit))]) for i in range(3)
]
...
metrics.dump_json(
    os.path.join(f"results/evaluation_results/{cfg.result_dir}", "metrics.json")
)
