...
def train_score(
    ...
    for epoch in range(score_agent.clock.epoch, cfg.n_epochs):
        ...
        for i, batch_sample in enumerate(pbar):
            ...
            losses = score_agent.train_func(
                data=batch_sample, gf_mode="score", teacher_model=teacher_model
            )
...
def train_energy(
    ...
    if ranking is False:
        ...
        train_score(
            cfg, train_loader, val_loader, test_loader, energy_agent, teacher_model
        )
    else:
        for epoch in range(energy_agent.clock.epoch, cfg.n_epochs):
            ...
            for i, batch_sample in enumerate(pbar):
                ...
                pred_pose = score_agent.pred_func(
                    data=batch_sample, repeat_num=5, save_path=None
                )

                """ train energy """
                losses = energy_agent.train_func(
                    data=batch_sample, pose_samples=pred_pose, gf_mode="energy"
                )
                pbar.set_description(
                    f"EPOCH_{epoch}[{i}/{len(pbar)}][loss: {[value.item() for key, value in losses.items()]}]"
                )
            ...
            if energy_agent.clock.epoch % cfg.eval_freq == 0:
                ...
                for i in range(len(data_modes)):
                    ...
                    """ get pose samples from pretrained score network """
                    pred_pose = score_agent.pred_func(
                        data=test_batch, repeat_num=5, save_path=None
                    )
