...
def fps_down_sample(vertices, num_point_sampled):
    ...
    farthest = np.argmax(
        _d
    )  # Select the point farthest from the center of gravity as the starting point
...
def trans_form_quat_and_location(quaternion, location, quat_type="wxyz"):
    assert quat_type in [
        "wxyz",
        "xyzw",
    ], f"The type of quaternion {quat_type} is not supported!"
...
def get_pose_representation(batch_rot_mat, pose_mode):
    ...
    if pose_mode == "quat_xyzw":
        ...
        rot = matrix_to_rotation_6d(batch_rot_mat.permute(0, 2, 1)).reshape(
            batch_rot_mat.shape[0], -1
        )
    else:
        raise NotImplementedError
...
def transform_batch_pts(
    batch_pts, batch_pose, pose_mode="quat_wxyz", inverse_pose=False
):
    """
    Args:
    ...
    trans_mat = torch.cat(
        (
            trans_mat,
            torch.tile(torch.tensor([[0, 0, 0, 1]]).to(trans_mat.device), (B, 1, 1)),
        ),
        dim=1,
    )

    new_pts = copy.deepcopy(batch_pts)
    padding = torch.ones([batch_pts.shape[0], batch_pts.shape[1], 1]).to(
        batch_pts.device
    )
    pts = torch.cat((batch_pts[:, :, :3], padding), dim=2)
    new_pts[:, :, :3] = torch.matmul(
        trans_mat.to(torch.float32), pts.permute(0, 2, 1)
    ).permute(0, 2, 1)[:, :, :3]
