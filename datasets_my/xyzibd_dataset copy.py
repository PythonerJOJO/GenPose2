...
def compute_translation_stats(dataset):
    ...
    for i in range(len(dataset)):
        data = dataset[i]
        # 获取原始平移量
        raw_trans = data["label"]["translation"].numpy()
...
class XyzibdDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        roi_rgb, roi_depth, roi_mask, roi_K, roi_coord_2d = DataUtils.cut_roi(
            rgb, depth, mask_visib, mat_K, self.cfg
        )
        ...
        top_rgb, top_map = DataUtils.transformer_image_view(
            filter_rgb, valid_coords, np.asarray(pcd_main.points), roi_K, -20, "x"
        )
        left_rgb, left_map = DataUtils.transformer_image_view(
            filter_rgb, valid_coords, np.asarray(pcd_main.points), roi_K, 20, "y"
        )
        # 点云可视化（验证旋转正确性）测试用
        ...
        # )
        # 该归一化的归一化
        main_rgb_norm = DataUtils.rgb_transform(filter_rgb)
        ...
        label_dict["translation"] = torch.as_tensor(
            normalized_trans, dtype=torch.float32
        ).contiguous()
        ...
        data_dict = {
            ...
            "roi_center_dir": torch.rand(32, 3).contiguous(),
